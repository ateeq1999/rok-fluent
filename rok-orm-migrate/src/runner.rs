...
///
/// ```rust,ignore
/// use rok_orm_migrate::{MigrationRunner, FileSource, Migration, SchemaExecutor};
...
///     .source(rok_auth::migrations())          // embedded SQL migrations
///     .source(FileSource::new("./migrations")) // SQL file migrations
...
impl MigrationRunner {
    ...
    ///
    /// ```rust,ignore
    /// runner.migration(CreateUsersTable)
    /// ```
    ...
    ///
    /// Useful when collecting migrations from a `database/migrations/mod.rs`:
    ///
    /// ```rust,ignore
    /// runner.migrations(vec![
    ...
    ///
    /// A "gap" is a pending registered migration whose name sorts *before* the
    /// most recently applied migration.  This happens when a developer adds a
    /// migration with an old timestamp after newer ones have already been applied.
    ///
    ...
    pub async fn gap_warnings(&self) -> anyhow::Result<Vec<String>> {
        ...
        // Also warn about applied migrations no longer in the registered list (orphans).
    ...
    pub async fn run(&self) -> anyhow::Result<()> {
        self.ensure_table().await?;
        // Warn about version gaps before running.
        for w in self.gap_warnings().await? {
            eprintln!("rok-orm-migrate WARNING: {w}");
        }
