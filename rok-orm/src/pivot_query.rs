...
/// let tag_count = post.tags().count().await?;
    ...
    T: Model + for<'r> sqlx::FromRow<'r, PgRow> + Send + Unpin,
{
    ...
    /// `SELECT t.* FROM t INNER JOIN pivot ON pivot.rfk = t.pk WHERE pivot.fk = $1`
    ...
    /// `SELECT COUNT(*) FROM t INNER JOIN pivot ON pivot.rfk = t.pk WHERE pivot.fk = $1`
    ...
    pub async fn sync(self, ids: &[impl Into<SqlValue> + Clone]) -> Result<(), sqlx::Error> {
        ...
        // Delete pivot rows whose rfk is NOT in the new set
        ...
        // Insert missing rows ON CONFLICT DO NOTHING
    ...
    pub async fn toggle(self, ids: &[impl Into<SqlValue> + Clone]) -> Result<(), sqlx::Error> {
        ...
        // Find which ids already exist in the pivot
        let placeholders: Vec<String> = (2..=id_vals.len() + 1).map(|i| format!("${i}")).collect();
        let sel_sql = format!(
        ...
        for id_val in id_vals {
            ...
            if already_exists {
                ...
                let ins =
                    format!(
                    "INSERT INTO {through} ({fk}, {rfk}) VALUES ($1, $2) ON CONFLICT DO NOTHING",
                    through = self.through, fk = self.fk, rfk = self.rfk,
                );
