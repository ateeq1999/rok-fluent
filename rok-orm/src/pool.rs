...
//!
//! [`OrmLayer`](crate::orm_layer::OrmLayer) sets the pool automatically for
...
//! use rok_orm::pool;
//!
//! pool::with_pool(db_pool.clone(), async {
...
///
/// Call this at application startup before serving requests.  Named pools are
/// then addressable via [`ModelQuery::on`](crate::model_query::ModelQuery::on).
///
/// # Example
///
/// ```rust,ignore
/// let analytics_pool = PgPool::connect(&analytics_url).await?;
...
/// Run `f` with `pool` available to all [`ModelQuery`] terminals in scope.
///
...
///
/// Read queries that have `use_replica = true` (the default) will be routed
/// to the replica pool.  Write queries always use the primary pool.
...
///
/// Used internally by [`OrmLayer`] to avoid an extra `.await` at the call site.
///
/// [`OrmLayer`]: crate::orm_layer::OrmLayer
#[cfg(feature = "axum")]
...
///
/// When `use_replica` is true and a replica pool is in scope, the replica pool
/// is returned.  Otherwise the primary pool is returned.
...
where
    T: rok_orm_core::Model + for<'r> sqlx::FromRow<'r, sqlx::postgres::PgRow> + Send + Unpin,
{
...
pub async fn count<T: rok_orm_core::Model>(builder: QueryBuilder<T>) -> Result<i64, sqlx::Error> {
    let pool = resolve_read_pool(&builder)?;
    executor::count(&pool, builder).await
}

/// Aggregate query, routing to replica when `builder.use_replica` is true.
pub async fn aggregate<T: rok_orm_core::Model>(
...
///
/// ```rust,ignore
/// let m = rok_orm::pool::snapshot(&pool);
/// println!("pool size={} idle={} active={}", m.size, m.idle, m.active);
/// ```
...
/// each labelled with `pool = label`.  Wire up a recorder (e.g.
/// `metrics-exporter-prometheus`) before calling this.
///
/// # Example
///
/// ```rust,ignore
/// // In your Prometheus scrape endpoint:
/// rok_orm::pool::emit_metrics(&pool, "primary");
/// ```
#[cfg(feature = "metrics")]
