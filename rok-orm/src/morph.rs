...
///
/// ```rust,ignore
/// // Comment has fields commentable_type: String, commentable_id: i64
/// let morph = MorphTo::new(&comment.commentable_type, comment.commentable_id);
    ...
    T: Model + for<'r> sqlx::FromRow<'r, PgRow> + Send + Unpin,
{
    ...
    pub async fn sync(self, ids: &[impl Into<SqlValue> + Clone]) -> Result<(), sqlx::Error> {
        ...
        // Delete rows whose related_fk NOT IN ids
...
mod tests {
    ...
    fn singular_regular() {
        ...
        assert_eq!(naive_singular("tags"), "tag");
    }

    #[test]
    fn singular_ies_ending() {
        assert_eq!(naive_singular("categories"), "category");
        assert_eq!(naive_singular("countries"), "country");
    }

    #[test]
    fn singular_no_change() {
        assert_eq!(naive_singular("data"), "data");
        assert_eq!(naive_singular("media"), "media");
    }
    ...
    #[test]
    fn morph_many_count_sql() {
        let q = MorphMany::<Comment>::new("post", 1i64, "commentable");
        let (sql, _) = q.count_sql();
        assert!(sql.starts_with("SELECT COUNT(*)"), "sql={sql}");
        assert!(sql.contains("commentable_type = $1"), "sql={sql}");
    }
    ...
    fn morph_to_many_select_sql() {
        ...
        assert!(sql.contains("taggable_id = $2"), "sql={sql}");
        assert_eq!(params.len(), 2);
        assert!(matches!(&params[0], SqlValue::Text(s) if s == "post"));
    }

    #[test]
    fn morph_to_many_count_sql() {
        let q = MorphToMany::<Tag>::new("post", 1i64, "taggables", "taggable", "tag_id");
        let (sql, _) = q.count_sql();
        assert!(sql.starts_with("SELECT COUNT(*)"), "sql={sql}");
    ...
    #[test]
    fn morph_to_many_select_sql_video_side() {
        let q = MorphToMany::<Tag>::new("video", 99i64, "taggables", "taggable", "tag_id");
        let (sql, params) = q.select_sql();
        assert!(matches!(&params[0], SqlValue::Text(s) if s == "video"));
        assert!(matches!(&params[1], SqlValue::Integer(99)));
        let _ = sql; // SQL shape is the same; only params differ
    }
}
